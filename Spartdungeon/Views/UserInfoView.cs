    ...
    public class UserInfoView
    {
        ...
        public int PrintUserInfo(Player user)
        {
            ...
            do
            {
                ...
                string text = bonusAttack > 0 ? $"+{bonusAttack.ToString("0.#")}" : bonusAttack.ToString("0.#");
                Console.WriteLine($"공격력: {user.Status.FinalAttack} ({text})");
                float bonusDefense = user.Status.BonusDefense;
                text = bonusDefense > 0 ? $"+{bonusDefense.ToString("0.#")}" : bonusDefense.ToString("0.#");
                Console.WriteLine($"방어력: {user.Status.FinalDefense} ({text})");
                Console.WriteLine($"체력: {user.Status.CurrHealth}/{user.Status.MaxHealth}");
